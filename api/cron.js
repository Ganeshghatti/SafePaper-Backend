...
constEncryptionService = require('../utils/encryption');
...
const runCronJob = async () => {
  try {
    ...
    console.log(`Found ${exams.length} scheduled exams without decoded questions`);
      ...
      if (timeDiff <= 5 && timeDiff > 0) {
        ...
        for (const question of exam.selectedQuestions) {
          try {
            ...
            console.log(`Combining ${relevantKeys.length} guardian keys for question ${question._id}`);
        ...
        if (decodedQuestions.length > 0) {
          console.log(`Successfully decoded ${decodedQuestions.length} questions`);
